...
# Page config
st.set_page_config(page_title="Falcon RVD",page_icon="↗️")
st.title('Falcon RVD ↗️')

AI71_API_KEY = st.secrets["api_key"] # replace with your own key
client = AI71(AI71_API_KEY)


# Falcon LLM Call
@st.cache_data
def falcon_rvd_api(user_query):
...
semantic vector search process by using LLMs like Falcon-180B
to prioritize terms in the query and then matching for similar vectors
in the order of prioritized terms. 

The terms may be from the query or generated by the `LLM (Falcon-180B)` to
better encapsulate the intent of the query. The embedding model used is Falcon2.

These terms will be weighed and pairwise similarity will be considered to obtain the final scores and ranking.
This ensures **high quality results**/top-matches.
...
if st.button("Vector Search!"):
	if ta_value == "" or uq_value == "":
		...
		model = load_model()
		llm_response = falcon_rvd_api(user_query)
		...
		st.subheader("Falcon Generated Priority Terms")
